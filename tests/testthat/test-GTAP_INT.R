...
for (d in seq_along(data_prefix)) {
  ...
  test_that(paste(db, paste(model, "partial uniform full swap scenario")), {
  ...
  test_that(paste(db, paste(model, "partial uniform part swap scenario")), {
  ...
  test_that(paste(db, paste(model, "partial uniform explicit year")), {
    ...
    expect_true(all(
      outputs$dat$aoall[REGr == "asia" & PROD_COMMj == "crops" & Year == year]$Value == -1,
      outputs$dat$aoall[REGr != "asia" & PROD_COMMj != "crops" & Year != year]$Value == 0
    ))
  })
