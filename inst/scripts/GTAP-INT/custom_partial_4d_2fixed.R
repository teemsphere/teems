# 4D variable (afeall: ENDW_COMMi x PROD_COMMj x REGr x ALLTIMEt), 2 dims fixed
# reduce_shock picks free_idx=[REGr, ALLTIMEt] (k=2, no blk)
# write_ragged: dcast row=ALLTIMEt ~ col=REGr -> 4x3 matrix
# stride ordering: antidim([REG,ALT]) = [1,3] -> write_order=[ALT,REG] (col=REG, row=ALT)
# old broken code would produce 3x4 (REG rows x ALT cols) -- detectable mismatch
...
# Fix ENDW_COMMi="natlres" AND PROD_COMMj="crops"; free dims REGr(3) x ALLTIMEt(4), n=12
# reduce_shock: all 3-element free subsets need >=48 rows; size-2 [3,4] factor=12 matches -> k=2
afeall <- afeall_full[
  afeall_full$ENDW_COMMi == "natlres" & afeall_full$PROD_COMMj == "crops",
]
