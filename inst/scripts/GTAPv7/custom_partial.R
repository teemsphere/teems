...
# 2D: remove rows that break complete-group formation for every free_idx subset
...
# 3D: remove by two simultaneous dim conditions — breaks all potential complete groups
...
# 4D: deterministic removal — remove all (REGs=="chn", REGd=="usa") tuples
...
atall <- atall_full[
  !(atall_full$REGs == "chn" & atall_full$REGd == "usa"),
]
