# 4D variable (afeall: ENDW_COMMi x PROD_COMMj x REGr x ALLTIMEt), 1 dim fixed
# reduce_shock picks free_idx=[ENDW_COMMi, REGr, ALLTIMEt] (k=3, blk=1 col)
# write_ragged: blk=ALLTIMEt(4), row=REGr(3), col=ENDW_COMMi(4) -> 4 slices of 3x4
# stride ordering: antidim([ENDW,REG,ALT]) = [1,4,12] -> write_order=[ALT,REG,ENDW]
# old broken code would order [ENDW,REG,ALT] -> blk=ENDW, row=REG, col=ALT -> 4 slices of 3x4
# but with wrong dim assignment causing incorrect values
...
# Fix PROD_COMMj="crops"; free dims are ENDW_COMMi(4) x REGr(3) x ALLTIMEt(4), n=48
# reduce_shock: free_idx=[1,3,4] factor=48 is the only 3-element subset not including PROD
#   that matches n_free=48 -> one het_grp -> k=3
afeall <- afeall_full[afeall_full$PROD_COMMj == "crops", ]
