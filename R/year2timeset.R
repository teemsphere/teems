...
.year2time_set.uniform <- function(shk,
  ...
  if ("Year" %in% names(shk$subset)) {
    Year <- purrr::pluck(shk, "subset", "Year")
    time_set_upper <- intersect(shk$ls_upper, int_set_names)
    time_set <- shk$ls_mixed[match(time_set_upper, shk$ls_upper)]
    ...
    purrr::pluck(shk, "subset", "Year") <- purrr::pluck(sets, "ele", time_set_upper, v_idx)
    names(shk$subset) <- sub("Year", time_set, names(shk$subset))
  }
  return(shk)
}