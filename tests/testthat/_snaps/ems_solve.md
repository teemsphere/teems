...
# ems_solve errors when solution errors detected
    ...
    Condition
      Error in `ems_solve()`:
      x Errors detected during solution. See '<data_dir>/solve/solve_err_error/out/solver_out_HHMM.txt'.
...
# ems_solve errors when solution singularity detected
    ...
    Condition
      Error in `ems_solve()`:
      x Singularity detected during solution. See '<data_dir>/solve/solve_err_sing/out/solver_out_HHMM.txt'.
...
# ems_solve informs terminal run
    ...
    Message
      ...
      docker run --rm --mount type=bind,src=<data_dir>/solve/solve_info_terminal,dst=/opt/teems teems:latest /bin/bash -c "/opt/teems-solver/lib/mpi/bin/mpiexec -n 1 /opt/teems-solver/solver/hsl -cmdfile /opt/teems/GTAPv7.cmf -matsol 0  -regset REG  -nsubints 1 -solmed Johansen -nesteddbbd 0 -presol 1 -laA 300 -laDi 500 -laD 200 -maxthreads 1 -nox 2>&1 | tee /opt/teems/out/solver_out_HHMM.txt"
      ...
      diagnostic output:
      '<data_dir>/solve/solve_info_terminal/out/solver_out_HHMM.txt'.
      4. If no errors or singularities are detected, use the following expression to
      structure solver binary outputs:
      `ems_compose("<data_dir>/solve/solve_info_terminal/GTAPv7.cmf")`
